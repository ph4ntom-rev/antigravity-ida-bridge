...
def find_ida() -> str:
    """Reliable search for IDA Pro binary."""
...
def cmd_launch(binary_path: str, client: BridgeClient):
    """Safe execution of IDA Pro with permission interception (fixes silent crash)."""
    ...
    if "error" not in client.ping():
        return format_output(
            {"status": "already_online", "info": client.info()})

    ida_exe = find_ida()
    if not ida_exe:
        return format_output(
            {"error": "IDA Pro not found. Set IDA_DIR env variable or add to PATH."})
    ...
    # [FIX] User is now notified if they lack admin rights to write the plugin
    if plugin_src.exists() and not plugin_dst.exists():
        try:
            ...
            return format_output(
                {"error": f"Permission denied installing plugin to {plugin_dst}. Run as Admin/Root."})

    log_file = Path.home() / ".antigravity_ida.log"
    proc = subprocess.Popen([ida_exe,
                             "-A",
                             f"-L{log_file}",
                             str(target)],
                            stdout=subprocess.DEVNULL,
                            stderr=subprocess.DEVNULL)
    return format_output({"status": "launched", "pid": proc.pid, "ida": str(
        ida_exe), "next": "Run: python cli.py wait"})
...
def main():
    ...
    parser.add_argument(
        "--url",
        help="Bridge server URL override",
        default=None)
    ...
    if args.command == "launch":
        ...
        while time.time() - start < args.timeout:
            if "error" not in client.ping():
                return format_output(
                    {"online": True, "waited": round(time.time() - start, 1)})
            time.sleep(2)
