...
def _setup_secure_token():
    ...
    try:
        ...
        # O_EXCL prevents symlink attacks; 0o600 ensures only the owner can
        # read/write.
        ...
        ida_kernwin.msg(
            f"[Antigravity] Security Warning: Could not set strict permissions on token: {e}\n")
    return token_path
...
def require_hexrays(func):
    ...
    def wrapper(*args, **kwargs):
        if not HAS_HEXRAYS:
            raise APIError("Hex-Rays decompiler is not available.", 501)
        return func(*args, **kwargs)
    return wrapper
...
def parse_ea(val: str) -> int:
    try:
        ...
        return int(
            val, 16) if val.startswith("0x") else int(
            val, 10 if val.isdigit() else 16)
    except ValueError:
...
def api_pseudocode(req, ea):
    ...
    if not cfunc:
        raise APIError("Decompilation returned None")

    sv = cfunc.get_pseudocode()
    lines = [ida_lines.tag_remove(sv[i].line) for i in range(sv.size())]
    lvars = [{"name": lv.name, "type": str(
        lv.type()), "is_arg": lv.is_arg_var} for lv in getattr(cfunc, 'lvars', [])]
    return {
        "ea": hex(ea),
        "name": ida_funcs.get_func_name(ea),
        "pseudocode": "\n".join(lines),
        ...
        try:
            if op == "rename-func":
                ...
                ok = ida_name.set_name(
                    ea, mut["name"], ida_name.SN_NOWARN | ida_name.SN_FORCE)
                if ok:
                    rollback.append(("rename", ea, old_name))
            ...
            for action in reversed(rollback):
                if action[0] == "rename":
                    ida_name.set_name(
                        action[1], action[2], ida_name.SN_NOWARN | ida_name.SN_FORCE)
            raise APIError(
                f"Batch failed at index {i}: {e}. State rolled back.")
...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def _check_auth(self) -> bool:
        if not AUTH_ENABLED:
            return True
        if self.path.rstrip('/') in ('/api/ping', '/api/schema'):
            return True
        ...
        self._send_response(
            {"error": "Unauthorized. Missing or invalid Bearer token."}, 401)
        return False
    ...
    def handle_request(self, method):
        ...
        if method == "POST":
            ...
            if content_length > MAX_BODY_SIZE:
                return self._send_response(
                    {"error": "Payload Too Large (DoS Protection)"}, 413)
            if content_length > 0:
                try:
                    req_data = json.loads(
        ...
        try:
            ...
            # Mask raw crash, but provide useful feedback for Agent
            # self-correction
            self._send_response({"error": "Internal Server Error", "details": str(
...
def stop_server():
    ...
    if _server:
        ...
        _thread = None
        ida_kernwin.execute_sync(
            lambda: ida_kernwin.msg("[Antigravity] Server OFFLINE.\n"),
            ida_kernwin.MFF_WRITE)
...
class AntigravityPlugin(ida_idaapi.plugin_t):
    ...
    def init(self):
        ida_kernwin.msg(
            "[Antigravity] Loaded. Press Ctrl+Shift+A to toggle server.\n")
