...
import ida_auto
import ida_lines
import ida_xref  # noqa: F401  # noqa: F401
import ida_typeinf
try:
    import ida_range
except ImportError:
...
def get_global_vars(max_count=500):
    ...
    def _inner():
        import ida_ida
        _ = ida_ida.inf_get_min_ea() if hasattr(ida_ida, 'inf_get_min_ea') else 0
        _ = ida_ida.inf_get_max_ea() if hasattr(ida_ida, 'inf_get_max_ea') else 0xFFFFFFFF
        gvars = []
        count = 0
        for ea, name in idautils.Names():
            if count >= max_count:
